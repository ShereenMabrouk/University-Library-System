...
public class AddStudents {
	...
	private void initialize() {
		...
		JButton addLibrarian = new JButton("Add Student");
		...
		addLibrarian.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				emS = em.getText();
				cityS = city.getText();
				contactNoS = contactNo.getText();
				addressS = address.getText();
				LogicStudents s = new LogicStudents();
				if (nameS.equals("") || passwordS.equals("") || emS.equals("") || addressS.equals("")
						|| cityS.equals("") || contactNoS.equals(""))
					check.setText("All fields are needed");
				else {
					ValidationOfData valid = new ValidationOfData();
					String validData = valid.checkValidity(idS, nameS, passwordS, emS, addressS, cityS, contactNoS);
					if (validData.equals("add")) {
						int x = s.add(idS, nameS, passwordS, emS, addressS, cityS, contactNoS);
						if (x == 1)
